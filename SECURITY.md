...
### In Scope
- Key storage security (IndexedDB, key isolation between origins)
- Key export and recovery flows (AES-GCM encryption, PBKDF2 parameters)
- Guardian recovery flow — threshold bypass or PII exposure
- ConsentToken signing UI — payload manipulation before signing
- Authentication and session handling
- Smart contract interactions (BEORegistry, IEORegistry, AccessControl, DomainRegistry)
- bsp-registry-api authentication, authorization, relay security
...
5. **Aptos permanence** — consent history is immutable and auditable forever
...
- On-chain registry: Aptos (Move modules); encrypted payload storage: Arweave
