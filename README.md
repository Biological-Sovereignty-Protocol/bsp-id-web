...
- **Gasless onboarding** — users register `.bsp` domains at zero cost; the operator wallet funds Aptos gas on their behalf
...
| On-chain ops | Move smart contracts on Aptos |
...
- An Aptos operator wallet with sufficient APT balance (for gasless relay)
...
|---|---|---|---|
| `NEXT_PUBLIC_APTOS_NETWORK` | Required | Aptos network to target | `testnet`, `mainnet`, or `devnet` |
| `NEXT_PUBLIC_BSP_CONTRACT_ADDRESS` | Required | Address of the deployed BSP Move module on Aptos | `0x1234...` |
| `BSP_REGISTRY_URL` | Required | Base URL of the BSP Registry API | `https://api.biologicalsovereigntyprotocol.com` |
| `APTOS_BALANCE_ALERT_THRESHOLD` | Optional | Minimum APT balance before an alert fires | `1.0` |
...
| `NEXT_PUBLIC_EXPLORER_URL` | Optional | Aptos explorer base URL for transaction links | `https://explorer.aptoslabs.com/txn/` |
...
- **Operator relay.** The operator wallet only signs Aptos gas transactions. It cannot read or modify user identity data, which is protected by the user's own Ed25519 key pair.
